...
const PortfolioComponent = ({ title , trik }: { title: string , trik:Trik[] }) => {
  ...
  const getGridClass = () => {
    if (["social media", "reels", "media production", "mobile application", "media buying" , "branding"].includes(normalizedTitle)) {
      return "xl:grid-cols-3 md:grid-cols-2 grid-cols-1";
    } 
    if (["web development", "seo"].includes(normalizedTitle)) {
      return "grid-cols-1";
    } 
    return "grid-cols-3"; 
  };
  ...
  normalizedTitle === "mobile application"
    ? "h-full"
    : "h-auto";
  return (
